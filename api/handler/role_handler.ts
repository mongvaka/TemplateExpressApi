...
let getSqlCreate = (req: Request) => {
  try {
    ...
    return (
      "INSERT INTO role_table (" + keys + ") values (" + index + ") RETURNING *"
    );
  } catch (e: any) {
    throw new Error(e.message);
  }
...
let getSqlDelete = () => {
  try {
    return "UPDATE role_table SET ( is_active , update_by , update_date ) = ( $1 , $2 , $3 ) WHERE role_uuid = $4 RETURNING *";
  } catch (e: any) {
    throw new Error(e.message);
  }
};
let getSqlDropdown = (req: Request) => {
  try {
    return "SELECT * FROM vw_role_table_Dropdown " + mTools.getCondition(req);
  } catch (e: any) {
    throw new Error(e.message);
  }
};
...
let getSqlList = (req: Request) => {
  try {
    return "SELECT * FROM vw_role_table_list " + mTools.getCondition(req);
  } catch (e: any) {
    throw new Error(e.message);
  }
};
let getSqlView = () => {
  try {
    return "SELECT * FROM vw_role_table_item WHERE role_uuid = $1";
  } catch (e: any) {
    throw new Error(e.message);
  }
};
...
let toCreateModel = (req: Request) => {
  try {
    ...
    model.role_name = body["role_name"];
...
let getLabel = (model: any) => {
  try {
    return model["role_name"] + " - " + model["remark"];
  } catch (e: any) {
    throw new Error(e.message);
  }
};
